...
def read_file():
    with open("./no_coord_stations.txt", "r", encoding="utf-8") as file:
        ...
        for i, line in enumerate(no_geo_stations):
            if '(' in line:
                no_geo_stations[i] = line[line.index('.')+1:line.index('(')]
            elif '.' in line:
                no_geo_stations[i] = line[line.index('.')+1:-1]
            else:
                no_geo_stations[i] = line[:-1]

        for i, line in enumerate(no_geo_stations):
            if '.' in line:
                no_geo_stations[i] = line[line.index('.')+1:]
...
def get_geo_information(stations):
    ...
    for station in stations:
        ...
        try:
            geo_info.append(
                [station, js_result[0]['lat'], js_result[0]['lon']])
        except IndexError:
